...
function App() {
  const [dark, setDarkVersion] = useState(false);
  ...
  const getSearchSettings = (settings) => {
    getCountPictures(settings);
  };

  async function getCountPictures(settings) {
    if (settings) {
      await axios
      ...
      <div className="container">
        <div className="header">
          ...
          <svg
            ...
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className={dark ? "light-btn dark" : "light-btn"}
            onClick={screenVersion}
          >
            <path
        ...
        <SearchPanel getSearchSettings={getSearchSettings} darkVersion={dark} />
        <HttpContent
          newPage={page}
          authorIdSet={settings.authorId}
          locationIdSet={settings.locationId}
          searchNameSet={settings.searchName}
          rangeFrom={settings.rangeFrom}
          rangeBefore={settings.rangeBefore}
          darkVersion={dark}
        />
        <Paginator getPage={getPage} countPictures={count} darkVersion={dark} />
      </div>
    </div>
  );
}
