...
export default class App extends Component {
  ...
  constructor(props) {
    ...
    this.getSearchSettings = (settings) => {
      this.getCountPictures(settings)
    }
  }

  async getCountPictures(settings) {
      if (settings) {
        await axios.get('https://test-front.framework.team/paintings', {
          params: {
              authorId: settings.authorId,
              locationId: settings.locationId,
              q: settings.searchName,
              created_gte: settings.rangeFrom,
              created_lte: settings.rangeBefore
          }
          })
          .then((res) => {
  ...
  componentDidMount() {
    this.getCountPictures()
  }


  render() {
    return (
      <div className={this.state.dark ? "content dark" : "content"}>
        <div className="container">
          <div className="header">
            <svg width="68" height="68" viewBox="0 0 68 68" fill="none" xmlns="http://www.w3.org/2000/svg" className="icon">
