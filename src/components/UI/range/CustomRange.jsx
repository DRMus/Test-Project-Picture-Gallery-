...
function CustomRange({getRange, darkVersion}) {
  ...
  return (
    <div className={(showBlock ? 'custom custom_range range_is_open' : 'custom custom_range') + (darkVersion ? " range__dark" : "")}>
      <div className="range__click" onClick={() => setShowBlock(!showBlock)}></div>
      <div className="range__input">
        <div className="range__placeholder">
          <span className="range__placeholder__value">Created</span>
          <svg 
            className={showBlock ? 'range__placeholder__svg arrow__open' : 'range__placeholder__svg'} 
            viewBox='0 0 20 20' 
